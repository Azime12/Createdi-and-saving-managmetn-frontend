...
const WithdrawModal: React.FC<WithdrawModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (numericAmount > parseFloat(selectedAccount.balance)) {
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <div className="flex justify-between items-center text-sm text-gray-500 mt-1">
                      <span>Available: {formatCurrency(parseFloat(selectedAccount?.balance || '0'))}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex justify-end space-x-3">
                  ...
                  <button
                    type="submit"
                    disabled={isLoading}
                    className={`px-4 py-2.5 text-sm font-medium rounded-lg text-white transition bg-green-600 hover:opacity-90`}
                  >
                    {isLoading ? (
